...
#
#     f = wave.open(f, 'wb')
#     f.setparams((nchannels, sampwidth, framerate, nframes, 'NONE',
#                  'not compressed'))
#
...
#         frames = b''.join(b''.join(struct.pack('h', int(max_amplitude *
#                                                          sample))
...
def write_wavefile(w, samples, nframes=None, nchannels=2, sampwidth=2,
    ...
    # performance)
    for chunk in grouper(bufsize, samples):
        return b''.join(b''.join(struct.pack('h', int(max_amplitude *
                                                        sample))
